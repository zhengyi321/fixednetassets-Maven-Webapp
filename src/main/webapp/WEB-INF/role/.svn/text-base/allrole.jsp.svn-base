...
<DIV>
  ...
  <TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
    ...
    vAlign=top align=middle>
          <DIV>
	   		<form action="/fixednetassets/role/add.do" align="left">
      ...
      border=1>
              <TBODY>
                <TR>
                  ...
                  <TH class=gridViewHeader scope=col>角色键值</TH>
                  <TH class=gridViewHeader scope=col>角色描述</TH> 
                  <TH class=gridViewHeader scope=col>编辑</TH> 
                </TR>
                <c:forEach items="${roleList }" var = "role">
                <TR>
                  <TD class=gridViewItem><input type="checkbox" name="checks" value="${role.guid }" /></TD>                  
                  <TD class=gridViewItem>${role.roleName }</TD>
                  <TD class=gridViewItem>${role.roleSign }</TD>
                  <TD class=gridViewItem>${role.description }</TD> 
                  <TD align="center" >
                	编辑/查看               
            	  </TD>           
                </TR>
         ...
         <a href='/fixednetassets/role/index.do?page=1' >首页</a>  
        <c:choose>
		<c:when test="${pages.pageNow - 1 > 0}">
			<a href="/fixednetassets/role/index.do?page=${pages.pageNow - 1}">上一页</a>
		</c:when>
		<c:when test="${pages.pageNow - 1 <= 0}">
			<a href="/fixednetassets/role/index.do?page=1">上一页</a>
		</c:when>
	</c:choose>
	<c:choose>
		<c:when test="${pages.pageNow + 1 < pages.totalPageCount}">
			<a href="/fixednetassets/role/index.do?page=${pages.pageNow + 1}">下一页</a>
		</c:when>
		<c:when test="${pages.pageNow + 1 >= pages.totalPageCount}">
			<a href="/fixednetassets/role/index.do?page=${pages.totalPageCount}">下一页</a>
		</c:when>
	</c:choose>
	
	
         <a href='/fixednetassets/role/index.do?page=${pages.totalPageCount}' >最后一页</a>    
         </div>
          </DIV>
        </TD>
...
//]]>
		...
		//	alert(checked);
			$.post("/fixednetassets/role/delete.do?guid="+checked, 
			function(data)
			{
		//		alert(data);
				if(data == true)
				{
				 	window.location.href="/fixednetassets/role/index.do?page=1";
				}
			}); 
		}
	})				
