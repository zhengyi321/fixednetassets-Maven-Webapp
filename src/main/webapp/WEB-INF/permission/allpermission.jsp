...
<DIV>
  ...
  <TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
    ...
    vAlign=top align=middle>
          <DIV>
	   		<form action="/fixednetassets/permission/add.do" align="left">
      ...
      border=1>
              <TBODY>
                <TR>
                  ...
                  <TH class=gridViewHeader scope=col>权限键值</TH>
                  <TH class=gridViewHeader scope=col>权限描述</TH>
                  <TH class=gridViewHeader scope=col>url</TH>
                ...
                <TR>
                  ...
                  <TD class=gridViewItem>${permission.permissionSign }</TD>
                  <TD class=gridViewItem>${permission.description }</TD>
                  <TD class=gridViewItem>${permission.url }</TD>
         ...
         <a href='/fixednetassets/permission/index.do?page=1' >首页</a>  
			<c:choose>
				<c:when test="${pages.pageNow - 1 > 0}">
					<a href="/fixednetassets/permission/index.do?page=${pages.pageNow - 1}">上一页</a>
				</c:when>
				<c:when test="${pages.pageNow - 1 <= 0}">
					<a href="/fixednetassets/permission/index.do?page=1">上一页</a>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${pages.pageNow + 1 < pages.totalPageCount}">
					<a href="/fixednetassets/permission/index.do?page=${pages.pageNow + 1}">下一页</a>
				</c:when>
				<c:when test="${pages.pageNow + 1 >= pages.totalPageCount}">
					<a href="/fixednetassets/permission/index.do?page=${pages.totalPageCount}">下一页</a>
				</c:when>
			</c:choose>
	
	
         <a href='/fixednetassets/permission/index.do?page=${pages.totalPageCount}' >最后一页</a>    
         </div>
          </DIV>
        </TD>
...
//]]>
	$('#allselect').click(
 	...
 	$("#delete").click(function(){
		...
		//	alert(checked);
			$.post("/fixednetassets/permission/delete.do?guid="+checked, 
			function(data)
			{
		//		alert(data);
				if(data == true)
				{
				 	window.location.href="/fixednetassets/permission/index.do?page=1";
				}
			}); 
		}
	})		
