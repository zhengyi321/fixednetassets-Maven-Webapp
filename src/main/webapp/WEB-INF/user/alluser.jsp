...
<DIV>
  ...
  <TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
    ...
    vAlign=top align=middle>
          <DIV>
	   		<form action="/fixednetassets/user/add.do" align="left">
      ...
      border=1>
              <TBODY>
                <TR>
                  ...
                  <TH class=gridViewHeader scope=col>ip</TH>
                  <TH class=gridViewHeader scope=col >编辑/查看</TH> 
                </TR>
                <c:forEach items="${userList }" var = "user">
                <TR>
                  <TD class=gridViewItem><input type="checkbox" name="checks" value="${user.guid }" /></TD>                  
                  <TD class=gridViewItem>${user.name }</TD>
                  <TD class=gridViewItem ><fmt:formatDate value="${user.createTime}" pattern="yyyy-MM-dd HH:mm" /></td>
                  <TD class=gridViewItem>${user.state }</TD>
                  <TD class=gridViewItem>${user.ipAddress }</TD>
                  <TD align="center" >
                	编辑/查看               
            	  </TD>           
                </TR>
         ...
         <a href='/fixednetassets/user/index.do?page=1' >首页</a>  
			<c:choose>
				<c:when test="${pages.pageNow - 1 > 0}">
					<a href="/fixednetassets/user/index.do?page=${pages.pageNow - 1}">上一页</a>
				</c:when>
				<c:when test="${pages.pageNow - 1 <= 0}">
					<a href="/fixednetassets/user/index.do?page=1">上一页</a>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${pages.pageNow + 1 < pages.totalPageCount}">
					<a href="/fixednetassets/user/index.do?page=${pages.pageNow + 1}">下一页</a>
				</c:when>
				<c:when test="${pages.pageNow + 1 >= pages.totalPageCount}">
					<a href="/fixednetassets/user/index.do?page=${pages.totalPageCount}">下一页</a>
				</c:when>
			</c:choose>
	
	
         <a href='/fixednetassets/user/index.do?page=${pages.totalPageCount}' >最后一页</a>    
         </div>
          </DIV>
        </TD>
...
//]]>
		...
		//	alert(checked);
			$.post("/fixednetassets/user/delete.do?guid="+checked, 
			function(data)
			{
		//		alert(data);
				if(data == true)
				{
				 	window.location.href="/fixednetassets/user/index.do?page=1";
				}
			}); 
		}
		})		
