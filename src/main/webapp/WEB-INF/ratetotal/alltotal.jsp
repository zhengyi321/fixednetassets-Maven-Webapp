  ...
  <TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
    ...
    vAlign=top align=middle>
          <DIV>
			...
			<DIV> <SPAN id=lblDep></SPAN>当前月份为: ${currentMonth } </DIV>
			</form>
      ...
      border=1 >
              <TBODY>
                <TR>
                  ...
                  <TH  colspan=4>固定资产原值</TH>
                  <TH  colspan=2>所属科目性质</TH> 
                ...
                <TR >                                   
                  ...
                  <TD class=gridViewItem>${fixed.balanceNum }</TD>
                  <TD class=gridViewItem>${fixed.balancePrice }</TD>
                  <TD class=gridViewItem>${fixed.salvage }</TD>
                  <TD class=gridViewItem>${fixed.manageFee }</TD>
                  <TD class=gridViewItem>${fixed.madeFee }</TD>
                  <TD class=gridViewItem>${fixed.sumDepreciationCurrentmonth }</TD>
                  <TD class=gridViewItem>${fixed.sumDepreciationTotal }</TD>
                  <TD class=gridViewItem>${fixed.fixedNetWorth }</TD>  
        ...
        <a href='/fixednetassets/ratetotal/index.do?page=1' >首页</a>  
        <c:choose>
		<c:when test="${pages.pageNow - 1 > 0}">
			<a href="/fixednetassets/ratetotal/index.do?page=${pages.pageNow - 1}">上一页</a>
		</c:when>
		<c:when test="${pages.pageNow - 1 <= 0}">
			<a href="/fixednetassets/ratetotal/index.do?page=1">上一页</a>
		</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${pages.pageNow + 1 < pages.totalPageCount}">
				<a href="/fixednetassets/ratetotal/index.do?page=${pages.pageNow + 1}">下一页</a>
			</c:when>
			<c:when test="${pages.pageNow + 1 >= pages.totalPageCount}">
				<a href="/fixednetassets/ratetotal/index.do?page=${pages.totalPageCount}">下一页</a>
			</c:when>
		</c:choose>
	
	
         <a href='/fixednetassets/ratetotal/index.do?page=${pages.totalPageCount}' >最后一页</a>    
         </div>
          </DIV>
        </TD>
...
//]]>
	...
	$("#delete").click(function(){
		...
		if(confirm("确认删除？"))
		{
			...
			$.post("/fixednetassets/ratetotal/delete.do?guid="+checked, 
			function(data)
			{
		//		alert(data);
				if(data == true)
				{
				 window.location.href="/fixednetassets/ratetotal/index.do?page=1";
				}
			}); 
		}
	})
	$("#export").click(function(){
		alert("hello");
		window.location.href="/fixednetassets/ratetotal/export.do";
	})
	function edit(data){
//		alert(data);
		window.location.href="/fixednetassets/ratetotal/edit.do?guid="+data;
	}
	function detail(data){
		...
		var url = "";
		url = "/fixednetassets/ratetotal/select.do?guid="+data;//弹出窗口的页面内容		
 	//	alert($(this).text());
